...
#############
gse20292 <- getGEO("GSE20292",GSEMatrix=TRUE)  #gets the matrix file with information about samples from the GEO website
pData <- pData(phenoData(gse20292[[1]]))[,10:12]
colnames(pData)[1] <- "case"
colnames(pData)[2] <- "age"
colnames(pData)[3] <- "gender"
pData$case <- gsub("disease state: Parkinsons disease", "PD", pData$case)
...
##########
GSE1 = pData[c(2:8,10, 18, 21,22),] 
GSE2 = pData[c(1, 9, 11:17, 19, 20, 23:29),]
...
###########
untar("GSE20292_RAW.tar")
cels = list.files(pattern = "CEL")
sapply(paste(cels, sep="/"), gunzip)
cels = list.files(pattern = "CEL")
i <- cels[c(2:8,10, 18, 21,22)]
p <- cels[c(1, 9, 11:17, 19, 20, 23:29)]
cels <- c(i, p)
...
dev.off()
###-The density plo does not display any bimodalities or any other particular anomalies and there is a great deal of overlap among the individual density plots. This suggests good quality in the chips.
...
write.table(data.rma.exprs, file = "GSE20292_rma_data.txt", quote = F)
...
nrow(data.rma.exprs2)#6478


##############
#filtering out NA genes
##############
...
nrow(data.rma.exprs.varmed) # 5943
...
############
#for genes mapped to multiple probes, keep probe that is highest effect size
############

t  = var.medlimma$t #t statistic
n1 = 11 # number of cases
n2 = 18 #number of controls
...
GSE20292 <- list(expression.mat = limma.rma1, samples.info = df, design = design)
save(GSE20292, file = "GSE20292.Rdata")
